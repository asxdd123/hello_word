...
public abstract class DateUtils {
	...
	public static Date parseDate(String d,String type) {
		try {
			return new SimpleDateFormat(type).parse(d);
		} catch (ParseException e) {
			logger.error(e.getMessage());
		}
		return null;
	...
	public static String getBeforeMonth(Date date) {
		...
		return DateUtils.convertDate2String("yyyyMM", result.getTime());
	}
	...
	public static Boolean isFirstDayOfMonth() {
		String dateStr = DateUtils.convertDate2String("dd", new Date());
	 ...
	 * @return month
	 */
	public static String getBeforeWeek(String time) {
		Calendar result = Calendar.getInstance();
		result.setTime(parseDateTime(time));
		result.add(Calendar.DATE, -7);
		return DateUtils.convertDate2String(DATETIME_FORMAT, result.getTime());
	}
